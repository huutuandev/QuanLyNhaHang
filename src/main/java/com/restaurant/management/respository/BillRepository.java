...
public interface BillRepository extends JpaRepository<BillEntity, Long> {
    ...
    @Query("SELECT new com.restaurant.management.DTO.MonthlyRevenueDTO(MONTH(b.paidAt), SUM(b.paidAmount)) " +
            "FROM BillEntity b WHERE YEAR(b.paidAt) = :year GROUP BY MONTH(b.paidAt)")
    List<MonthlyRevenueDTO> sumMonthlyRevenue(@Param("year") int year);
